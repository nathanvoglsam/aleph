...
impl ImguiFont {
    pub fn init(
        ...
        let sources = [FontSource::TtfData {
            data: crate::embedded::data::fonts::cascadia_code(),
            size_pixels: 16.0,
            config: Some(config),
        }];
    ...
    pub fn create_sampler(device: &vulkan_core::Device) -> Sampler {
    ...
    pub fn create_image_view(device: &vulkan_core::Device, image: Image) -> ImageView {
    ...
    pub fn upload_font(&self, device: &vulkan_core::Device, allocator: &Allocator, data: &[u8]) {
        //
    ...
    pub unsafe fn destroy(&self, device: &vulkan_core::Device, allocator: &Allocator) {
