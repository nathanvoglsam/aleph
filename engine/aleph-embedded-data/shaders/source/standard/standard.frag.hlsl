...
float4 main(in StaticMeshPixelInput input) : SV_Target0 {
    ...
    // Light parameters
    const float3 light_position = float3(1.0,0.5,1.0);
    const float lumens = 200;
    ...
    const float roughness = RemapRoughness(0.5);
    ...
    const float3 brdf = ClearCoatBRDF(v, l, n, diffuse_colour, roughness, f0, 1.0, 0.01);
